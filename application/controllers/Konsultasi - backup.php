...
class Konsultasi extends CI_Controller {
	...
	function index() {
		...
		$config = array(
			array('field' => 'nama_pasien', 'label' => 'Nama Pasien', 'rules' => 'required'),
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data = $this->load->view('isi_dulu',  
				array('session_id_valid' => $_SESSION['session_id_valid'])
			, TRUE);
			$this->load->view('template', array('data'=>$data));
		} else {
			$sql = "INSERT INTO tmp_pasien(nama_pasien, jenis_kelamin, alamat, session_id, tanggal) values (?, ?, ?,?,?)";
			$status = $this->db->query($sql, array(@$_POST['nama_pasien'], @$_POST['jenis_kelamin'], @$_POST['alamat'], @$_POST['session_id_valid_requiered'], date("Y-m-d H:i:s"))
			);
			if($status) {
	...
	function analisa() {
		...
		$sql = "INSERT INTO analisa_hasil(nama_pasien, jenis_kelamin,  alamat, session_id, tanggal, kd_penyakit) SELECT nama_pasien, jenis_kelamin, alamat, session_id, tanggal, ? AS kd_penyakit FROM tmp_pasien WHERE session_id = ? AND session_id not in ( select session_id from analisa_hasil)";
